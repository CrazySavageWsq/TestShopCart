    ...
    if (number <= _maxValue) {
        ...
        if (_decreaseHide && number==_minValue) {
            ...
            [UIView animateWithDuration:0.3f animations:^{
                wself.decreaseBtn.alpha = 1;
                wself.decreaseBtn.frame = CGRectMake(0, 0, self->_height, self->_height);
            } completion:^(BOOL finished) {
                wself.textField.hidden = NO;
            }];
        }
...
- (void)decrease
{
    ...
    if (number >= _minValue) {
        ...
        if (_decreaseHide && number<_minValue) {
            ...
            [UIView animateWithDuration:0.3f animations:^{
                self->_decreaseBtn.alpha = 0;
                self->_decreaseBtn.frame = CGRectMake(self->_width-self->_height, 0, self->_height, self->_height);
            }];
            
            return;
        }
        if (_shakeAnimation) { [self shakeAnimationMethod]; }
        PPLog(@"数量不能小于%.1f",_minValue);
    }
}
